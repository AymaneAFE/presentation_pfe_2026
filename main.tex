...
\begin{frame}{Gestion des incidents - Alerte et Incident}
    \begin{columns}[c] % [c] = centré verticalement ; [T] pour aligner par le haut
        ...
        \begin{column}{0.50\textwidth}
            \footnotesize
            \begin{block}{Exemple : hôte \texttt{ubuntu-target} éteint}
                Une panne réelle injectée : l'hôte
                \texttt{ubuntu-target} (192.168.10.130) est
                volontairement éteint. Centreon détecte les ressources
                en défaut et génère \textbf{cinq alertes} :
                \begin{enumerate}
                    \item Vérification d'hôte (\textit{ping})
                    \item Vérification de service \textit{Ping} dédiée
                    \item Trois sondes SNMP \textit{Load}
                    \item \textit{Swap}
                    \item \textit{Memory}
                \end{enumerate}
                \vspace{0.3em}
                Un \textbf{seul incident source} à l'origine de toutes ces
                alertes : d'où la nécessité de les \textbf{corréler}.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
